...
- 공식 원문 링크 우선
- 가능하면 보조 출처 1개 이상 확보
- 브라우저로 본 내용도 최종 글에는 클릭 가능한 마크다운 링크로 남길 것
- 추출이 부실하거나 본문 확인이 안 되는 소스는 굳이 넣지 말 것
- 수치/가격/벤치마크는 원문 확인 전 단정 금지
- 루머는 루머라고 명시

## 5. 게시 품질 기준
다음 중 2개 이상 충족 못 하면 더 조사 후 게시:
- 오늘의 **새로운 뉴스**가 실제로 반영됨
- 오늘의 큰 흐름이 명확함
...
새로운 뉴스가 부족하면 오래된 내용을 억지로 부풀리기보다, 더 조사하거나 포스트 분량을 줄이더라도 새로운 정보 중심으로 쓴다.
